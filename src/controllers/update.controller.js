...
class HomeStayUpdate extends BaseController{
    async showFormUpdate(req,res){
       ...
       let homestay = await homestayToUpdate.updateHomestay(id)
        ...
        data = data.replace('{Bedrooms}', `<input type="number" name ="bedroomsUpdate" class="form-control" value="${homestay[0].numRestRoom}" placeholder="Update Bedrooms">`);
        ...
        data = data.replace('{Toilet}', `<input type="text" name="toiletUpdate" class="form-control" value="${homestay[0].numWCroom}" placeholder="Update Toilet">`);
        data = data.replace('{Describes}', `<input type="text" name="descriptionUpdate" class="form-control" value="${homestay[0].description}" placeholder="Update Describes">`);
